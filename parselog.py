...
def extract_data(framework_, topology_, csv_file_name_, log_file_names, print_on_success = True, custom_log_tags = None, replace_or_add_log_tags = "add", ip_address_ = "localhost"):
    ...
    setup_logging(framework_, topology_, csv_file_name_, ip_address_, custom_log_tags, replace_or_add_log_tags)
...
def setup_logging(framework_, topology_, csv_file_name_, ip_address_, custom_log_tags = None, replace_or_add_log_tags = "add"):
    ...
    global csv_data
    global csv_file_name, framework, topology, ip_address
...
def parse_logs(log_file_name):
    ...
    logs = load_logs(log_file_name, ip_address)#logs is an array of logs. Each log is a string. So it's an array of strings.
...
def write_logs():
    global csv_data
    write_file(str(csv_data), csv_file_name, ip_address)#, local_only = True)
    
    
    
def main():
    extract_data("tf", "resnet", "theOutputCsv2.csv", ["mylog"])
    """
